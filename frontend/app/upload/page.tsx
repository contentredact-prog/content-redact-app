...
import { uploadFile, getWork } from "@/lib/api";
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const poll = setInterval(async () => {
        attempts++;
        try {
          const work = await getWork(result.work_id);
          if (work.processing_status === "protected") {
            clearInterval(poll);
            setState("done");
          } else if (work.processing_status === "failed") {
            clearInterval(poll);
            setState("error");
    ...
    <main className="min-h-screen flex flex-col items-center justify-center p-8 bg-black text-white">
      <div className="w-full max-w-lg">
        ...
        <div
          ...
          `}
        >
          <input ref={fileInputRef} type="file" accept=".mp4,.mp3,.wav" className="hidden"
